...
//
// Präzision statt Dateiname-Raten: gesucht wird gezielt "Schadenzahlung"
// (maschinell benannte Versicherer-Zahlungsschreiben); das Aktenzeichen kommt
// aus dem Ordnerpfad .../Gutachten/JJJJ/MM/<az>/, nicht aus der LLM-Ausgabe.
...
const search = node({
  ...
  config: {
    name: 'Suche Schadenzahlung', position: [220, 300],
    parameters: { resource: 'file', operation: 'search', query: 'Schadenzahlung' },
...
const candidates = node({
  ...
  config: {
    ...
    parameters: { language: 'javaScript', jsCode: `const out = [];
for (const it of $input.all()) {
  ...
  out.push({ json: { fileId: x.id, name: x.name, webUrl: x.webUrl, folderAz } });
}
return out;` }
  }
});
